...
temp = sapply(10^seq(-2,2,0.25), function(i){
  c(i, og.alg(i), pureLap.alg(i), true.alg(i))
})
...
ggplot(long_data, aes(x = epsilon, y = value, color = solution)) +
  ...
  scale_color_manual(
    values = c("solution1" = "#F8766D", "solution2" = "#00BA38", "solution3" = "#619CFF"),  # Keep colors
    labels = c("solution1" = "No Noise", "solution2" = "Pure Noise", "solution3" = "Optimal")  # Change legend labels
  ) +
  labs(
    title = "",
